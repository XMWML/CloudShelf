...
2. 点击左下角 **New Connection** 或工具栏第一个加号。
3. 填写连接名称、协议、主机、端口、用户名和远端根目录。
4. FTP/FTPS/WebDAV 选择 Password 并输入密码；SFTP 可以选择 Password、SSH Agent 或 Private Key。
5. 点 **Save**。连接会出现在左侧列表，并立即尝试连接。成功后在左侧选中它即可浏览远端文件。
...
3. 选择本地文件夹后，设置远端目录、同步方向和执行频率，点击 **Add Rule**。
