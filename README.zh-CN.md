...
- 每个连接可在“同步管理”中添加、编辑、启用、停用、删除和立即执行多条本地文件夹同步规则；支持只上传、只下载、双向保留较新版本，也可从远端目录浏览器选择目标文件夹。
...
3. 在“同步管理”中添加本地文件夹，选择远端目录、同步方向和执行频率；可勾选“检测到本地文件夹变化后自动同步”。
4. 工具栏“同步”会立即执行全部启用规则；管理窗口也可单独运行某一规则。应用运行期间会按设定周期执行，已启用变更检测的上传或双向规则会在本地变化稳定两秒后自动执行。

同步策略默认很保守：不会自动删除本地或远端文件。没有可靠修改时间的服务器会以文件大小为准，避免每轮重复传输。WebDAV 创建目录返回 `405 Method Not Allowed` 时，应用会重新列出父目录；确认目录已存在便继续同步，不需要第二次手动执行。
